...
class MainEvent {
	...
	**/
	public function stop() {
		if (f == null){
			__.log().trace('empty f');
			return;
		}
		f = null;
		nextRun = Math.NEGATIVE_INFINITY;
		if (prev == null){
			__.log().trace('prev is null');
			__.log().trace('$next');
			@:privateAccess MainLoop.pending = next;
			__.log().trace('${@:privateAccess MainLoop.pending}');
		}else{
			__.log().trace('$next');
			prev.next = next;
		}
...
class MainLoop {
	...
	public static function hasEvents() {
		...
		while (p != null) {
			if (p.isBlocking){
				__.log().trace('has events');
				return true;
			}
			p = p.next;
		}
		//__.log().trace('has NO events');
		return false;
	}
	...
	**/
	...
	static function injectIntoEventLoop(waitMs:Int) {
		#if (target.threaded && !cppia)
		__.log().trace('injectIntoEventLoop');
			mutex.acquire();
			if(eventLoopHandler != null) {
				__.log().trace('cancel eventLoopHandler');
